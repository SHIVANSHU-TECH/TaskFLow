...
import { register as registerService, googleAuth } from '../services/authService';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 relative overflow-hidden px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute bottom-[-10%] left-[-10%] w-64 h-64 md:w-96 md:h-96 bg-pink-300/30 rounded-full blur-3xl opacity-50 mix-blend-multiply animate-bounce-short"></div>
        <div className="absolute top-[-10%] right-[-10%] w-64 h-64 md:w-96 md:h-96 bg-indigo-300/30 rounded-full blur-3xl opacity-50 mix-blend-multiply animate-bounce-short" style={{ animationDelay: '1.5s' }}></div>
      </div>

      {/* Success Popup */}
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-fade-in px-4">
          <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl flex flex-col items-center animate-fade-in-up max-w-sm w-full">
            ...
            <p className="text-slate-500 mt-2 text-center text-sm">Redirecting you to Sign In...</p>
          </div>
        </div>
      )}
      ...
      <div className="glass-card w-full max-w-md p-6 sm:p-8 rounded-2xl shadow-2xl z-10 animate-fade-in-up">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Create Account</h2>
          <p className="text-slate-500 mt-2 text-sm sm:text-base">Join TaskFlow today</p>
        </div>
        ...
        <div className="relative my-6 sm:my-8">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-4 bg-white dark:bg-slate-800 text-slate-400">Or sign up with</span>
          </div>
        </div>
